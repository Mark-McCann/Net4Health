...
#write.csv(pooled.results.table, file = "pooled estimates_6100.csv")


#for (sch in 1:6){
#  save(list = paste0("baseline.pooled.6100.sch",sch), file = paste0("baseline_pooled_6100_sch",sch,".rdata") )
#  save(list = paste0("control.pooled.6100.sch",sch), file = paste0("control_pooled_6100_sch",sch,".rdata") )
#}
...
for (sch in 1){
  pdf(paste0("mcmc_diag_baseline_sch",sch,".pdf"))
    for (iter in iternums){    
      ...
      ideg <- gof(baseline.6100.models[[iter]][[sch]]~idegree, control.gof.ergm(parallel=16, parallel.type="MPI"))
      plot(ideg)
      odeg <- gof(baseline.6100.models[[iter]][[sch]]~odegree, control.gof.ergm(parallel=16, parallel.type="MPI"))
      plot(odeg)
      tricen <- gof(baseline.6100.models[[iter]][[sch]]~triadcensus, control.gof.ergm(parallel=16, parallel.type="MPI"))
      plot(tricen)
    }
  dev.off()
}
...
for (sch in 1:6){
  pdf(paste0("mcmc_diag_control_sch",sch,".pdf"))
    for (iter in iternums){    
      mcmc.diagnostics(control.6100.models[[iter]][[sch]])
      ideg <- gof(control.6100.models[[iter]][[sch]]~idegree, control.gof.ergm(parallel=16, parallel.type="MPI"))
      plot(ideg)
      odeg <- gof(control.6100.models[[iter]][[sch]]~odegree, control.gof.ergm(parallel=16, parallel.type="MPI"))
      plot(odeg)
      tricen <- gof(control.6100.models[[iter]][[sch]]~triadcensus, control.gof.ergm(parallel=16, parallel.type="MPI"))
      plot(tricen)
    }
  dev.off()
}
